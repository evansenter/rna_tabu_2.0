...
/*@unused@*/
static char rcsid[] UNUSED = "$Id: foldMod.c,v 1.1 2009/02/04 18:35:14 lorenzwi Exp $";
...
/*---------------------------------------------------------------------------*/
PUBLIC short encode_seq_ext(const char *sequence){
...
PUBLIC int stack_energy_ext(int i, const char *string)
{
  return stack_energy(i, string);
}
...
PUBLIC int ML_Energy_fold(int i, int is_extloop){
  return ML_Energy(i, is_extloop);
}
...
eReturn getBarrierEnergy(char* sequence, char *start_str, char *end_str, short route[1000][500], int *rl, int init_w, int th){
  float eout;
  int energy, e2, emax, bestEmax, estart, eend, counter, bestE2;
  int i, x, num_changes, add_flag;
  short *start, *end, *add, *subtract, *bestSub, *bestAdd, *bestStart, *initStart, *initAdd, *initSub;
  char empty[500];
  static initialized=0;
  float efloat;
  eReturn eret;
  int **lock, n, tenure;
  int best_num_changes, init_num_changes;
  int nonImpr=0;
  int k,l;
  int ok = 0;
  float initW=(float) init_w/10;
  float w=initW;
  int rc=0;
  int best_rc;
  //  efloat=energy_of_struct(sequence, end_str);
  //  printf("efloat is %f.\n", efloat);
  if (initialized==0) length=encode_seq_ext(sequence);
  n=length;
  lock=(int**) malloc((n+1)*sizeof(int*));
  for(i=0;i<=n;i++)
    lock[i] = (int*) calloc(n+1,sizeof(int));

  pair_table = make_pair_table(start_str);
  start=pair_table;
  end=make_pair_table(end_str);
  for (i=0; i<length; i++)
    empty[i]='.';
  empty[i]='\0'; //end of string?
  add=make_pair_table(empty);
  bestStart=make_pair_table(empty);
  bestAdd =make_pair_table(empty);
  bestSub =make_pair_table(empty);
  initStart=make_pair_table(empty);
  initAdd =make_pair_table(empty);
  initSub =make_pair_table(empty);
  subtract=make_pair_table(empty);
  num_changes=0;
  for (i=1;i<=length;i++){
    if (start[i]<i || start[i]==end[i]) continue;
    subtract[i]=start[i];
    subtract[start[i]]=i;
	  initSub[i]=subtract[i];
	  initSub[start[i]]=i;
    num_changes++;
  }
  tenure=num_changes;
  memcpy(route[rc],start,(length+1)*sizeof(short));
  rc++;
  for(i=1;i<=length;i++)
    initStart[i]=start[i];
  for (i=1;i<=length;i++){
    if (end[i]<i || start[i]==end[i]) continue;
    add[i]=end[i];
    add[end[i]]=i;
	initAdd[i]=add[i];
	initAdd[end[i]]=i;
  ...
  //getchar();
  counter=0;
	init_num_changes = num_changes;
  //printf("num_changes is %d.\n", num_changes);
  //fflush(stdout);
  //printf("        ");
  //disp_ptable(start, "Start ");
  //  disp_ptable(start, "start");
  //  disp_ptable(end, "end");
  //  disp_ptable(add, "add");
  //  disp_ptable(subtract, "subtract");
  estart=e2=get_energy(start, sequence);
  //eend=get_energy(end,sequence);
  //eend = 0;
  //printf("Start e = %d, End ene = %d\n",estart,eend);
  emax=e2;
  ...
  while (num_changes>0){
    counter++;
    //printf("changes =%d counter =%d rc=%d\n barrier = %d",num_changes,counter,rc,emax-estart);
    //disp_ptable(start, "start");
    //getchar();
    if((rc>=500)||(counter>1000)){
      eret.start=estart;eret.end=e2;eret.max=100000;*rl=rc;
      for(i=0;i<=n;i++)
        free(lock[i]);
      free(lock);
      //printf("exit1\n");
    ...
    //printf("changes = %d, counter = %d, w = %f\n",num_changes,counter,w);
    //getchar();
    if(get_best_step(start, add, subtract, sequence, &i, &add_flag, &energy, lock,counter,w,num_changes,best_num_changes,emax,e2)){
     
     
      //    disp_ptable(start, "at beginning of loop, start");
      if (add_flag){
        lock[i][add_flag]=counter+((rand()%(2*init_num_changes-4)) +4);//tenure;
        add_ptable_bp(start, i, add_flag);
        if(add[i]==add_flag){
	       remove_ptable_bp(add, i, add[i]);
           num_changes--;
	 }else{
	     add_ptable_bp(subtract, i, add_flag);
	     num_changes++;
	 }
      } else {
        lock[i][start[i]]=counter+((rand()%(2*init_num_changes-4)) +4);//tenure;
        if(start[i]==subtract[i]){
     	  remove_ptable_bp(subtract, i, subtract[i]);
	  num_changes--;
        }else{
	  add_ptable_bp(add, i, add[i]);
	  num_changes++;
        }
        remove_ptable_bp(start, i, start[i]);
      }
      memcpy(route[rc],start,(length+1)*sizeof(short));
      rc++;
      e2+=energy;
      if (e2>emax) emax=e2;
      //printf("estart, emax, e2, th are %d, %d, %d. %d\n", estart, emax, e2, th);
      //getchar();
    }
    if(emax-estart>th){
       initW+=0.5;
	...
	start[k]=initStart[k];
	add[k]=initAdd[k];
	subtract[k]=initSub[k];
        bestStart[k]=start[k];
        bestAdd[k]=add[k];
        bestSub[k]=subtract[k];
				
       }
       emax =estart;
	e2= estart;
	num_changes=init_num_changes;
	 rc=1;
        best_num_changes=num_changes;
        bestEmax=emax;
        bestE2 = e2;
    ...
    if(num_changes>=best_num_changes){//last is actually working as best
      nonImpr++;
      //last_num_changes=num_changes;
      if(nonImpr >= n){
        w*=2;
		//restart best
		//printf("Restart best\n");
    ...
    // ok=1;
    //if(ok){
     //   printf("%3d %3d %4d %5d ", i, add_flag, energy, e2);
      //  disp_ptable(start, "start");
        //getchar();
    //}
    //    printf("num_changes is now %d.\n", num_changes);
    //printf("%8d", e2);
    //disp_ptable(start, "Start ");
  }
  //  printf("estart, emax, e2 are %d, %d, %d.\n", estart, emax, e2);
  //printf("******************\n");
  //printf("num_changes = %d\n ",num_changes);
  eret.start=estart;eret.end=e2;eret.max=emax;*rl=rc;
...
int disp_ptable(short* intable, char* name){
  
  int x;

  printf("%30s:", name);
  for (x=1;x<=intable[0];x++){
    if (intable[x]==0)
      printf(".");
    else if (intable[x]>x)
      printf("(");
...
int get_energy(short* table, char* sequence){
  int i, energy;

  energy =  ML_Energy(0, 1);
  for (i=1; i<=length; i++) {
    if (table[i]==0) continue;
    energy += stack_energy(i, sequence);
    i=table[i];
  }
  return energy;
}

int get_best_step(short* start, short* add, short* subtract, char *sequence,
  ...
  *emin=999999;
  ...
  //printf("length is %d.\n", length);
  for (i=1;i<=length;i++){
    //printf("get best step i, start[i], %d, %d.\n", i, start[i]);
    if(start[i]==0){
      for(j=i+1;j<=length;j++){
	
        if(start[j]==0 && j>i+3 && pair[S[i]][S[j]]){
          //printf("get best step i, j, lock[i][j], start[j] %d, %d %d %d.\n", i, j, lock[i][j], start[j]);
          //getchar();
          e0=find_energy_add2(start, i, j, sequence);
          //printf("energy = %d",e0);
	  //disp_ptable(start, "Start ");
	  //disp_ptable(add, "add ");
           if(j==add[i])
	     ...
	     e1=e0+w*100;
	   //printf("fitness is %d",e1);
           //getchar();
          if(((e1<e0)&&(nc-1<bc)&&(e2+e0<=emax))||(lock[i][j]<counter)){//Aspiration:improves best_num_changes and does not increase eb
            //Find place
            m=-1;
            do{
              m++;
            }while((m<max_ties)&&(e1>fits[m]));
           //move the rest
           //printf("It goes in %d \n",j);
           //getchar();
            if(m<max_ties){
              for(k=max_ties-1;k>m;k--){
                fits[k]=fits[k-1];
                ies[k]=ies[k-1];
                adds[k]=adds[k-1];
                enes[k]=enes[k-1];  
  ...
  // printf("get here?WE?E?W\n");

  for (i=1;i<=length; i++){
    //printf("before\n");
    //printf("start[i] is %d\n", start[i]);
    //printf("lock[i][start[i]] is %d\n", lock[i][start[i]]);
    //printf("sub-get best step i, j, lock[i][j], start[j] %d, %d %d %d.\n", i, start[i], lock[i][start[i]], start[start[i]]);
    if (start[i]<i) continue;
    //printf("after\n");
    e0=find_energy_sub(start, i, start[i], sequence);
    //printf("energy = %d",e0);
    //getchar();
    if(start[i]==subtract[i])
	  e1=e0-w*100;
    ...
    //printf("fitness is %d",e1);
    //getchar();
    if(((e1<e0)&&(nc-1<bc)&&(e2+e0<=emax))||(lock[i][start[i]]<counter)){//Aspiration:improves best_num_changes and does not increase eb
  ...
  if(nc==1){
    //printf("choosing l %d, e=%d\n",l,enes[l]);
    //printf("ener[0] = %d\n",enes[0]);
    //getchar();
    l=0;
   } 
  //printf("size(%d) - choose %d (w = %f): fitmin = %d emin = %d, index = %d, add = %d\n",size,l,w,fits[l],enes[l],ies[l],adds[l]);
  //getchar();
  *emin=enes[l];*return_index=ies[l];*addflag=adds[l];
  return 1;
}
...
int can_add(short* start, short* add, const int i){
  ...
  //want to add basepair (i, add[i])
  if (start[i]!=0) return 0;
  if (start[add[i]]!=0) return 0;
  for (ii=i-1; ii>0;ii--){
    if (start[ii]==0) continue;
    if (start[ii]<ii) {ii=start[ii];continue;}
    if (start[ii]<=add[i] && start[ii]>i){
      //checking if (ii, start[ii]) conflicts with (i, add[i])
      return 0;
    }
    else
      break;
  ...
  for (j=add[i]+1; j<=length;j++){
    if (start[j]==0) continue;
    if (start[j]>j) {j=start[j];continue;}
    if (start[j]>=i && start[j]<add[i]){
      //checking if (ii, start[ii]) conflicts with (i, add[i])
      //checking if (start[j], j) conflicts with (i, add[i])
      return 0;
    }
    else
      break;
...
int find_energy_add(short* start, short* add, const int i, char *sequence){
  ...
  //  disp_ptable(start, "at start of add, start is");
  for (ii=i-1; ii>0; ii--){
    if (start[ii]==0) continue;
    if (start[ii]<ii) {
      ii=start[ii];
  ...
  if (ii>0)
    estart=local_energy(ii, sequence);
  else
    estart=ML_Energy(0, 1);
  add_ptable_bp(start, i, add[i]);
  //  disp_ptable(start, "in middle, start is");
  if (ii>0)
    eend=local_energy(ii, sequence);
  else
    eend=ML_Energy(0,1);
  eend+=local_energy(i, sequence);
  remove_ptable_bp(start, i, add[i]);
  //  disp_ptable(start, "at end of add, start is");
  //  printf("energy change is %d.\n", eend-estart);
  return eend-estart;
}
...
int find_energy_sub(short* start, const int i, const int j, char *sequence){
  ...
  //  disp_ptable(start, "at start of sub, start is");
  for (ii=i-1; ii>0; ii--){
    if (start[ii]==0) continue;
    if (start[ii]<ii) {
      ii=start[ii];
  ...
  if (ii>0)
    ...
    estart=ML_Energy(0, 1);
  estart+=local_energy(i, sequence);
  remove_ptable_bp(start, i, j);
  //  disp_ptable(start, "in middle, start is");
  if (ii>0)
    eend=local_energy(ii, sequence);
    ...
    eend=ML_Energy(0,1);
  add_ptable_bp(start, i, j);
  //  disp_ptable(start, "at end of sub, start is");
  //  printf("energy change is %d.\n", eend-estart);
  return eend-estart;
}

int add_ptable_bp(short* table, const int i, const int j){
  table[i]=j;table[j]=i;
  return 0;
}
int remove_ptable_bp(short* table, const int i, const int j){
  table[i]=0;table[j]=0;
  return 0;
}

int local_energy(int i, const char* string)
{
  /* calculate energy of substructure enclosed by (i,j) */
  int ee;
  int j, p, q, type, type_2;
