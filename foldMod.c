...
/** ADDED FUNCTIONS **/
eReturn getBarrierEnergy(char* sequence, char *start_str, char *end_str, short route[1000][500], int *rl, int init_w, int th, int user_dangles){
  /** THIS LINE INITIALIZE DANGLES, BUT IT MUST BE A SIMPLIER WAY **/
  dangles=user_dangles;
  energy_of_struct(sequence, end_str);
  /** **/
  ...
  estart=e2=get_energy(start, sequence, verbosity_level);
  ...
  while (num_changes>0){
    ...
    if(get_best_step(start, add, subtract, sequence, &i, &add_flag, &energy, lock,counter,w,num_changes,best_num_changes,emax,e2)){
      ...
      if (add_flag){
	/** CORRECTED TO AVOID MODULE BY ZERO **/
	if(init_num_changes==2){
	        lock[i][add_flag]=counter+(rand() +4);//tenure;
	}
	else{
	...
	/** CORRECTED TO AVOID MODULE BY ZERO **/
	if(init_num_changes==2){
	        lock[i][start[i]]=counter+(rand() +4);//tenure;
	}
	else{
...
int get_energy(short* table, char* sequence, int verbosity_level){
  ...
  for (i=1; i<=length; i++) {
    ...
    energy += stack_energy(i, sequence, verbosity_level);
    i=table[i];
  }
  return energy;
}
...
/** REPLACED
    ...
    ee = MyLoopEnergy(p-i-1, j-q-1, type, type_2,
		      S1[i+1], S1[j-1], S1[p-1], S1[q+1]);
    if (eos_debug>0)
      printf("Interior loop (%3d,%3d) %c%c; (%3d,%3d) %c%c: %5d\n",
	     i,j,string[i-1],string[j-1],p,q,string[p-1],string[q-1], ee);
...
/** REPLACED
    ...
    ee = MyHairpinE(j-i-1, type, S1[i+1], S1[j-1], string+i-1);
    if (eos_debug>0)
      printf("Hairpin  loop (%3d,%3d) %c%c              : %5d\n",
	     i, j, string[i-1],string[j-1], ee);
  } else {
/** REPLACED
    ee = ML_Energy(i,0); */
    ee = energy_of_ml_pt(i,pair_table);
    if (eos_debug>0)
      printf("Multi    loop (%3d,%3d) %c%c              : %5d\n",
	     i,j,string[i-1],string[j-1],ee);
  }
...
/* FROM PLUGIN */
int MyLoopEnergy(int n1, int n2, int type, int type_2,
		      int si1, int sj1, int sp1, int sq1) {
